...
def loop():
    ...
    if location_ready:
        ...
        while True:
            log("ищем")
            for heroName in files:
                try:
                    location_hero = pyautogui.locateCenterOnScreen(f'{folder_path}/{heroName}',confidence=0.8)
                    log(f"{folder_path}/{heroName}")
                    if(location_hero):
                        choice(location_hero)
                        log("Найден")
                except pyautogui.ImageNotFoundException:
                    pass
                time.sleep(0.5)
    ...
    if IsWorking:
        root.after(1000, lambda: threading.Thread(target=loop, daemon=True).start())
